    ...
    while(1==1){
        for(int i=0; i<rowSize; i++){
            ...
            if(tree[cur].leftParent!=-1){
                tree[cur].leftCount=tree[tree[cur].leftParent].leftCount+tree[tree[cur].leftParent].rightCount+tree[tree[cur].leftParent].power;
            }
            if(tree[cur].rightParent!=-1){
                tree[cur].rightCount=tree[tree[cur].rightParent].rightCount+tree[tree[cur].rightParent].power;
            }
            cur++;
        }
        rowSize++;
...
int main(){
    ...
    for(;s>0;s--){
        long long canNumber;
        cin>>canNumber;
        long long res=tree[canNumber].leftCount+tree[canNumber].rightCount+tree[canNumber].power;
        cout<<res<<"\n";
    }
    delete [] tree;
}